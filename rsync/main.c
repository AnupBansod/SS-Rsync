...
{
	char *p = NULL, *shell_machine = NULL, *shell_user = NULL;
	char *http_server;
	...
	http_server = malloc(strlen(shell_machine));
	strcpy(http_server, shell_machine);
	if (DEBUG_GTE(CMD, 2)) {
...
<<<<<<< HEAD
		...
		fflush(stderr);
		if(enable_http == 1){
			http_pid = do_over_http(&f_in, &f_out, http_server);
...
<<<<<<< HEAD
					...
					read(to_child[0], buf, count); // scope for error handling
					post_size = count;
				}
...
int main(int argc,char *argv[])
{
	int ret;
	int orig_argc = argc;
	char **orig_argv = argv; // Aks : keep another copy of original arguments 
	//	int copy_orig_argc = argc ;  don't need a copy of arguments , but we will need it later
